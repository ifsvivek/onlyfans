...
def configure(
    temp_curve=None, speed_curve=None, idle_speed=None, poll_interval=None, view=False
):
    is_root()
...
def info():
    ...
    if FAN1_SPEED_FILE:
        print(f"Debug: FAN1_SPEED_FILE path: {FAN1_SPEED_FILE}")
    ...
    if FAN2_SPEED_FILE:
        print(f"Debug: FAN2_SPEED_FILE path: {FAN2_SPEED_FILE}")
    ...
    if BOOST_FILE:
        print(f"Debug: BOOST_FILE path: {BOOST_FILE}")
...
def set_fan_speed(arg1, arg2=None):
    is_root()
    load_ec_module()
    if os.path.isfile(IPC_FILE):
