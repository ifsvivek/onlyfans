...
# Handle IndexError for BOOST_FILE, FAN1_SPEED_FILE, and FAN2_SPEED_FILE
boost_files = glob.glob("/sys/devices/platform/hp-wmi/hwmon/*/pwm1_enable")
if boost_files:
    BOOST_FILE = boost_files[0]
else:
    BOOST_FILE = None

fan1_speed_files = glob.glob("/sys/devices/platform/hp-wmi/hwmon/*/fan1_input")
if fan1_speed_files:
    FAN1_SPEED_FILE = fan1_speed_files[0]
else:
    FAN1_SPEED_FILE = None

fan2_speed_files = glob.glob("/sys/devices/platform/hp-wmi/hwmon/*/fan2_input")
if fan2_speed_files:
    FAN2_SPEED_FILE = fan2_speed_files[0]
else:
...
@cli.command(name="bios-control", aliases=["b"])
...
@cli.command(name="boost", aliases=["x"])
@click.argument("arg", type=bool)
def boost_cli(arg):
    is_root()
    load_ec_module()
    if BOOST_FILE:
        ...
        print("  ERROR: Boost file not found")


@cli.command(name="configure", aliases=["config", "c"])
@click.option("--temp-curve", type=str)
@click.option("--speed-curve", type=str)
@click.option("--idle-speed", type=click.IntRange(0, 100))
@click.option("--poll-interval", type=click.FLOAT)
...
@cli.command(name="service", aliases=["e"])
...
def info_cli():
    ...
    if BOOST_FILE:
        ...
        print("  Fan Boost : Not available")


@cli.command(name="set", aliases=["s"])
...
@cli.command(name="version", aliases=["v"])
def version_cli():
    print("  OnlyFans ")
    print("  Version 1 ")
